...
3.  GCC compiles app.c to binary shared object (app.cpython-311-x86_64-linux-gnu.so)
4.  The .so file can be imported like a Python module but can't be easily reversed
5.  --inplace: Creates the .so file in the current directory alongside source
...
###### Copy everything from /app in builder stage.This includes:

- \*.so file (compiled Cython binary)
- run.py (entry point)
- Any other runtime files

##### **_ Note: Does NOT include app.py, app.c, setup.py (already deleted) _**
...
###### PYTHONPATH tells Python where to search for modules.Order matters - Python searches left to right:
